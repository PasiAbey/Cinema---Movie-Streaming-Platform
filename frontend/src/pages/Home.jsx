      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: S.muted, fontSize: "0.8rem" }}>
            ★ {item.vote_average?.toFixed(1)} · Rank #{currentIndex + 1}
          </span>
        </div>
        ...
        <div style={{ display: "flex", gap: "1rem", marginTop: "1.25rem", pointerEvents: "auto" }}>
          ...
          <button
            ...
            onClick={async () => {
              if (!user) return alert("Please Sign In first to save favorites!");
              setAddingFav(true);
              try {
            ...
            <div style={{
              ...
              <div
                onClick={async (e) => {
                  ...
                  try {
                    const token = await user.getIdToken();
                    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/user/progress/${item.type}/${item.id}`, {
                      method: 'DELETE',
                      headers: {
                        'Authorization': `Bearer ${token}`
                      }
